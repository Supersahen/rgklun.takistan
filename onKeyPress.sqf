...
keyboard_forward_tuning_handler = {
	...
	if( _vcl iskindof "LandVehicle") then {
		...
		switch _lvl do {
			...
			case 5: {
				_vcl setVelocity [(_vel select 0) * 1.009, (_vel select 1) * 1.009, (_vel select 2)];
			};
			case 6: {
				_vcl setVelocity [(_vel select 0) * 1.013, (_vel select 1) * 1.013, (_vel select 2)];
			};
			case 7: {
				_vcl setVelocity [(_vel select 0) * 1.040, (_vel select 1) * 1.040, (_vel select 2)];
			};
			case 8: {
...
combat_roll_check = {
	if(!INV_shortcuts) exitWith {false};
	_handguns = ["glock17_EP1","m9","m9sd","makarov","makarovsd","revolver_EP1","Colt1911","UZI_EP1","UZI_SD_EP1","Sa61_EP1"];

	_speed = speed player;

		if (_speed > 21) exitWith {

			_height = getPosATL player;

			[[], "mp_anim_sync", [player,"switchmove","ActsPercMrunSlowWrflDf_FlipFlopPara"]] call mp_aware_me;

			uiSleep 1.2;

			player setPosATL [ getPosATL player select 0, getPosATL player select 1, getPosATL player select 2 + 1];

			uiSleep 2;

			(format ["%1 playmove ""%2"";", player, "AmovPercMstpSrasWrflDnon"]) call broadcast;
	...
	_armed = _weapon in _handguns;

	if (_armed) exitWith {(format ["%1 playmove ""%2"";", player, "AovrPercMstpSnonWnonDf"]) call broadcast; true;};

	if ((!_armed) and (_speed < 21) and !(_weapon == "")) exitWith {(format ["%1 playmove ""%2"";", player, "AovrPercMstpSlowWrflDf"]) call broadcast; true;};

	if ((!_armed) and (_weapon == "")) exitWith {(format ["%1 playmove ""%2"";", player, "AovrPercMstpSnonWnonDf"]) call broadcast; true;};
...
KeyUp_handler = {
	...
	switch _key do {
		...
		case DIK_4: {
			...
			//player groupChat "Disabled due to abuse";
			//if (_inVehicle) exitWith {_handled=false;};
