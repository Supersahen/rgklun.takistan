...
if (_art == "serverloop") then {	
	...
	while {true} do {
		...
		for [{_i=0}, {_i < _iterations}, {_i=_i+1}] do {
			...
			_time_left = (chief_elections_term - _i);
		...
		for [{_i=0}, {_i < count(ChiefArray)}, {_i=_i+1}] do {
			...
			_player_variable = missionNamespace getVariable [_player_variable_name, objNull];
			if ( ((count (ChiefArray select _i)) > _MaxStimmen) and ([_player_variable] call player_exists)) then {	
				_MaxStimmen = (count (ChiefArray select _i));
				_MaxPos = _i;
			};
		};	
